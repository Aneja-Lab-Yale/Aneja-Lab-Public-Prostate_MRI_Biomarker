#!/usr/bin/env python3
# -------------------------------------------------------------------------------
# Prostate Cancer MRI Preprocessing tool
# Original Author: Samira Masoudi
# Original Date:   11.07.2020
# Modified by Sajid Hossain
# 5/17/2022
# Aneja-Lab
# -------------------------------------------------------------------------------
...
def arg_def():
    ...
    parser.add_argument('--dicom_folder', type=str,
                        default='C:\\Users\\shasa\\Desktop\\Prostate Cancer Research\\Prostate MRI\\Data\\Input\\Patients', help='Input path')
    parser.add_argument('--target_folder', type=str,
                        default='C:\\Users\\shasa\\Desktop\\Prostate Cancer Research\\Prostate MRI\\Data\\Preprocessed',
                        help='Output path')
    parser.add_argument('--mask_folder', type=str,
                        default='C:\\Users\\shasa\\Desktop\\Prostate Cancer Research\\Prostate MRI\\Data\\Input\\Masks',
    ...
    parser.add_argument('--desired_zero_padded_size', type=int, default=[120, 120, 100],
    ...
    parser.add_argument('--Normalization_type', type=str, default='Nyul',
    ...
    parser.add_argument('--Standardization_type', type=str, default='STND',
...
def main():
    ...
    with open(join(args.target_folder,'image_specifications.csv'), 'w', newline='') as csvfile:
        ...
        if args.image_type=='MRI':
            ...
            y+=1
            for i, patient in enumerate(Patient_List):
                Input_path = join(args.target_folder, 'Bias_field_corrected', patient+'.nii')
                image_t = NiftiRead(Input_path)
                data5 = sitk.GetArrayFromImage(image_t)
                z = data5.shape[0] // 2
                axarr[x,y].imshow(data5[z], cmap='gray')
                x += 1
            ...
            for i, patient in enumerate(Patient_List):
                ...
                data6 = sitk.GetArrayFromImage(image_B_S)
                z = data6.shape[0] // 2
                axarr[x,y].imshow(data6[z], cmap='gray')
                y += 1

                if args.Masking:
                    mask = NiftiRead(join(args.mask_folder,  patient + '.nii'))
                    image_B_S_m1 = Dicom_matmul(mask, image_B_S, 1)
                    image_B_S_m2 = Dicom_matmul(mask, image_B_S, 2)
                    image_B_S_m = Dicom_concatenate(image_B_S_m1, image_B_S_m2)
                    image_B_S_m_c=crop_2D_mask(image_B_S_m)
                else:
                    image_B_S_m_c = image_B_S

                if len(args.Spacing):
                        image_B_S_m_R  = Resample_3D(image_B_S_m_c, [args.Spacing[0], args.Spacing[1], args.Spacing[2]])
                else:
                    image_B_S_m_R = image_B_S_m_c
                ...
                data9 = sitk.GetArrayFromImage(image_B_S_m_R_Z)
                z = data9.shape[0] // 2
                axarr[x, y].imshow(data9[z], cmap='gray')
                x+=1
            ...
            cols = ['Raw Input', 'N4 Bias Correction', 'Normalized', 'Masked/Padded']
            rows = ['PMUB-0001', 'PMUB-0002', 'PMUB-0003', 'PMUB-0005', 'PX-0000', 'PX-0001']
